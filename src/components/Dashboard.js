...
export default function Dashboard({userDetail, validated, userIcon ,songState ,volumeIcon, volumeBackground, modifyClassTheme, modifySongRegion, songDetail, modifySongPlay, modifyIsPlaying, percentage, modifyPercentage, modifyCurruntTime, songCount, modifySongState, songs, ModifySongVolume}) {
    ...
    return (
        <div key={`${songDetail.songRegion} ${songDetail.classTheme}`} className="dashboard">
            ...
            {(validated)? <UserIcon userDetail={userDetail} validated={validated} userIcon={userIcon}/> : ((validated === undefined)? <ChangeList songRegion={songDetail.songRegion} modifySongRegion={modifySongRegion}></ChangeList> : <UserIcon userIcon={userIcon}/>)}
            {(validated === undefined)? <UserIcon className="user-icon-home" userIcon={"https://trunkey2003.github.io/general-img/default-profile-pic.jpg"}></UserIcon> : <></>}
